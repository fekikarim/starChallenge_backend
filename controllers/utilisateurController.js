...
class UtilisateurController {
    ...
    static async update(req, res, next) {
        ...
        try {
            console.log('Données reçues pour la mise à jour:', {
                id: req.params.id,
                body: { ...req.body, motDePasse: req.body.motDePasse ? '***' : 'non fourni' }
            });
            ...
            try {
                ...
                console.error('Erreur lors de la mise à jour dans la base de données:', updateError);
                return res.status(500).json({ 
                    success: false,
    ...
    static async delete(req, res, next) {
        ...
        logger.info(`Tentative de suppression de l'utilisateur avec l'ID: ${userId} par l'utilisateur: ${requestingUserId}`);
        ...
        try {
            ...
            logger.info(`Utilisateur ${userId} supprimé avec succès par ${requestingUserId}`);
            ...
            if (error.message && error.message.includes('FOREIGN KEY constraint failed')) {
                logger.warn(`Tentative de suppression d'un utilisateur référencé: ${userId}`);
                return res.status(409).json({
                    success: false,
                    message: 'Impossible de supprimer cet utilisateur car il est référencé par d\'autres données',
                    details: 'Veuillez d\'abord supprimer ou mettre à jour les références à cet utilisateur.'
                });
            }
